    ...
    def display(self):
        ...
        for x in range(len(self.li_val)):
            if self.justify == 'right':
                self.elements.append(self.canvas.create_text(self.x, self.y+space*x,
                                    text=str(x+1) + ". ", fill=self.font_color, font=("Times", 
                                    self.font_size), justify=self.justify, anchor=self.float_pos))

                self.elements.append(self.canvas.create_text(self.x-20, self.y+space*x,
